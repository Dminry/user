      ...
      if (isset($_GET["page"])) {
         switch ($check_url) {
            ...
            case "gb_add_gallery":
               ...
               $get_gallery_image_meta_data_unserialize = get_unserialize_data_gallery_bank("meta_id = $gallery_id and meta_key = %s", "image_data");
               $sort_order = array();
            ...
            case "gb_sort_galleries":
               ...
               if (isset($_REQUEST["gallery_id"])) {
                  ...
                  $sort_gallery_images = $wpdb->get_results
                      ...
                      $wpdb->prepare
                          (
                          "SELECT * FROM " . gallery_bank_meta() . " WHERE meta_id = %d and meta_key = %s", $gallery_id, "image_data"
                      )
                  );
            ...
            case "gb_manage_albums":
               $manage_albums_data_unserialize = get_unserialize_data_gallery_bank("meta_key = %s", "album_data");
               $count_manage_albums_data = count($manage_albums_data_unserialize);
               break;
            case "gb_add_album":
               $get_galleries_data_for_album = get_unserialize_data_gallery_bank("meta_key = %s", "gallery_data");
               break;
            case "gb_sort_albums":
            ...
            case "gb_image_browser_layout":
               $image_browser_layout_data = get_meta_value_gallery_bank("image_browser_layout_settings");
               break;
